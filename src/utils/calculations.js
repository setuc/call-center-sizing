 ...
 export const calculateTokens = (duration, selectedTask, contextTokens, realTimeInterval, wordsPerMinute, tokensPerWord) => {
  ...
  switch (selectedTask) {
    ...
    case 'realTimeSentiment':
      outputTokens = Math.ceil(duration / realTimeInterval) * 50;
      break;
    case 'complexAnalysis':
      outputTokens = Math.ceil(inputTokens * 0.3);
      break;
    default:
      outputTokens = Math.ceil(inputTokens * 0.2);
  }
  ...
  return {
    input: inputTokens + (contextTokens || 0),
    output: outputTokens,
    total: inputTokens + (contextTokens || 0) + outputTokens
  };
};

// Function to calculate costs
export const calculateCost = (callDistribution, durationDistribution, selectedModel, selectedSpeechModel, wordsPerMinute, selectedTask, contextTokens, realTimeInterval) => {
  ...
  callDistribution.forEach((calls, hour) => {
    const tokens = calculateTokens(durationDistribution[hour], selectedTask, contextTokens, realTimeInterval, wordsPerMinute, 1.5);
